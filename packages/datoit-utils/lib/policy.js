...
const searchLocalPolicy = (policy, plugin, apiName) => {
  ...
  let absoluteApi = _.get(datoit.api, absoluteApiName);
  ...
  const api = _.get(datoit.api, apiName);
...
const policyResolvers = [
  {
    ...
    get: policy => {
      ...
      return getPolicyIn(_.get(datoit, ['api', api]), policyName);
    },
  },
  {
    name: 'admin',
    ...
    get: policy => {
      return getPolicyIn(_.get(datoit, 'admin'), stripPolicy(policy, ADMIN_PREFIX));
    },
  },
  {
    name: 'plugin',
    ...
    get(policy) {
      ...
      return getPolicyIn(_.get(datoit, ['plugins', plugin]), policyName);
    },
  },
  {
    name: 'global',
    ...
    get(policy) {
      return getPolicyIn(datoit, stripPolicy(policy, GLOBAL_PREFIX));
    },
  },
];
