...
module.exports = {
  ...
  getNature: ({ attribute, attributeName, modelName }) => {
    ...
    const models = datoit.db.getModelsByPluginName(attribute.plugin);

    const pluginModels = Object.values(datoit.plugins).reduce((acc, plugin) => {
      return acc.concat(Object.values(plugin.models));
    }, []);

    const allModels = Object.values(datoit.models).concat(pluginModels);
  ...
  defineAssociations: function(model, definition, association, key) {
    try {
      ...
      const targetModel =
        targetName !== '*' ? datoit.db.getModel(targetName, association.plugin) : null;
      ...
      if (targetName !== '*') {
        const model = datoit.db.getModel(targetName, association.plugin);
      ...
      const pluginsModels = Object.keys(datoit.plugins).reduce((acc, current) => {
        Object.keys(datoit.plugins[current].models).forEach(entity => {
          Object.keys(datoit.plugins[current].models[entity].attributes).forEach(attribute => {
            const attr = datoit.plugins[current].models[entity].attributes[attribute];

            if ((attr.collection || attr.model || '').toLowerCase() === model.toLowerCase()) {
              acc.push(datoit.plugins[current].models[entity]);
            }
          });
        });
      ...
      const appModels = Object.keys(datoit.models).reduce((acc, entity) => {
        Object.keys(datoit.models[entity].attributes).forEach(attribute => {
          const attr = datoit.models[entity].attributes[attribute];

          if ((attr.collection || attr.model || '').toLowerCase() === model.toLowerCase()) {
            acc.push(datoit.models[entity]);
          }
        });
      ...
      const componentModels = Object.keys(datoit.components).reduce((acc, entity) => {
        Object.keys(datoit.components[entity].attributes).forEach(attribute => {
          const attr = datoit.components[entity].attributes[attribute];

          if ((attr.collection || attr.model || '').toLowerCase() === model.toLowerCase()) {
            acc.push(datoit.components[entity]);
          }
        });
