...
const getAssociationFromFieldKey = ({ model, field }) => {
  ...
  for (let i = 0; i < fieldParts.length; i++) {
    ...
    if (assoc) {
      association = assoc;
      tmpModel = datoit.db.getModelByAssoc(assoc);
      continue;
    }
  ...
  return whereClauses
    .filter(({ field, value }) => {
      if (_.isNull(value)) {
        ...
        datoit.log.warn(`The value of field: '${field}', in your where filter, is undefined.`);
        return false;
      }
      return true;
    })
...
const buildQuery = ({ model, filters = {}, ...rest }) => {
  ...
  return datoit.db.connectors.get(model.orm).buildQuery({ model, filters, ...rest });
};
