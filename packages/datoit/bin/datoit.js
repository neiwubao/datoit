...
const checkCwdIsStrapiApp = name => {
  ...
  try {
    ...
    if (!_.has(pkgJSON, 'dependencies.datoit')) {
...
const getLocalScript = name => (...args) => {
  ...
  const cmdPath = resolveCwd.silent(`datoit/lib/commands/${name}`);
  if (!cmdPath) {
    console.log(
...
// `$ datoit version` (--version synonym)
...
program
  .command('start')
  .description('Start your Datoit application')
...
// `$ datoit generate:template <directory>`
program
...
program
  ...
  .description('Builds the datoit admin app')
  .action(getLocalScript('build'));

// `$ datoit install`
program
  .command('install [plugins...]')
  .description('Install a Datoit plugin')
