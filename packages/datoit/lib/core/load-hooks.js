...
const loadPluginsHooks = async (plugins, hooks) => {
  for (let pluginName of plugins) {
    const dir = path.resolve(findPackagePath(`datoit-plugin-${pluginName}`), 'hooks');
...
const loadAdminHooks = async hooks => {
  ...
  const dir = path.resolve(findPackagePath('datoit-admin'), hooksDir);
  await loadHooksInDir(dir, hooks);
...
const loadHookDependencies = async (installedHooks, hooks) => {
  for (let hook of installedHooks) {
    const hookDir = path.dirname(require.resolve(`datoit-hook-${hook}`));
...
const mountHooks = (name, files, hooks) => {
  files.forEach(file => {
    ...
    let dependencies = [];
    try {
      dependencies = _.get(require(`datoit-hook-${name}/package.json`), 'datoit.dependencies', []);
    } catch (err) {
      // Silent
    }
